...
scan_lx = 0;
scan_ty=0.5;
//cout<<(int)round((scan_lx+(-left_limit_x)/dx)*screen_width+(scan_ty+ (-bottom_limit_y))/dy)<<endl;;
//cout<<z_buffer.buffer[(int)round((scan_lx+(-left_limit_x)/dx)*screen_width+(scan_ty+ (-bottom_limit_y))/dy)].z;
for(i=triangles.size();i>0;i--){
    triangle t = triangles.at(i-1);
    t.print();
   if(t.boundary_values[2] < bottom_y)
    ...
    //cout<<scan_ty<<"  "<<scan_by<<endl;
    double temp=0,temp2=0,dz;
    while(scan_ty >= scan_by){
        ...
        if(scan_zd == 0){
        cout<<"fault"<<endl;
        cout<<"2:"<<scan_ty<<" "<<t.c.x<<" "<<t.c.y<<" "<<t.b.x<<" "<<t.b.y<<" temp: "<<temp<<endl;
        cout<<"temp2: "<<temp2<<endl;
        cout<<"lx: "<<scan_lx<<" rx: "<<scan_rx<<" zu: "<<scan_zu<<" zd: "<<scan_zd<<endl;
        }*/
        ...
        /*
        if(scan_zd == 0){
                cout<<"fault"<<endl<<endl;
                    cout<<"3:"<<scan_ty<<" "<<t.c.x<<" "<<t.c.y<<" "<<t.a.x<<" "<<t.a.y<<" temp: "<<temp<<endl;
                        cout<<"temp2: "<<temp2<<endl;
        cout<<"lx: "<<scan_lx<<" rx: "<<scan_rx<<" zu: "<<scan_zu<<" zd: "<<scan_zd<<endl;
        }
*/
    }
        cout<<"FINAL: " <<"lx: "<<scan_lx<<" rx: "<<scan_rx<<" zu: "<<scan_zu<<" zd: "<<scan_zd<<endl;
        dz = (scan_zu - scan_zd)/((scan_rx - scan_lx)/dx);
        if(dz < -100)
            dz = 0;
        cout<<fixed<<setprecision(6)<<"dz: "<<dz<<endl<<endl;
        temp = scan_zd;

        while(scan_lx <=scan_rx){
            if( z_buffer.buffer[(int)round( ((scan_lx+(-left_limit_x))/dx) * screen_width + ((scan_ty+ (-bottom_limit_y))/dy)) ].z == temp)
                c1++;
            if(temp < z_far){
                z_buffer.buffer[(int)round( ((scan_lx+(-left_limit_x))/dx) * screen_width + ((scan_ty+ (-bottom_limit_y))/dy))].z=temp;
                z_buffer.buffer[(int)round( ((scan_lx+(-left_limit_x))/dx) * screen_width + ((scan_ty+ (-bottom_limit_y))/dy))].set_color(t.color[0],t.color[1],t.color[2]);
                count++;
            }
...
MyFile.open("z_buffer.txt");
double t1;
int t2=0,index;
for(int i=0;i<screen_width;i++){
        for(int j=0;j<scree_height;j++){
                index = (int)round(i*screen_width+j);
                if(z_buffer.buffer[index].z < z_far){
                     t2++;
                     t1 =  z_buffer.buffer[index].z;
                     image.set_pixel(i,j,z_buffer.buffer[index].rgb[0],z_buffer.buffer[index].rgb[1],z_buffer.buffer[index].rgb[2]);
                     MyFile<<fixed<<setprecision(6)<<t1<<" ";
               }
               MyFile<<" ";
        }
            MyFile<<endl;
    }
cout<<count<<" "<<t2<<" "<<c1<<endl;
