...
void drawBullet(){
    double c = -200;
    double k = (c - center.z)/center.z;
    point p ;
    p.x = center.x;
    p.y = center.y;
    p.z = center.z -65;
    point res = center + p * k;
    res.print();
}
	...
	switch(button){
		case GLUT_LEFT_BUTTON:
			if(state == GLUT_DOWN){		// 2 times?? in ONE click? -- solution is checking DOWN or UP
                point v;
                point cr;
                cr.z = -30 * cos(pi * b /180);
                cr.x = -30 * sin(pi * b/180) ;

                cr.z = cr.z * cos(pi * c/180);
                cr.y = 30 * sin(pi*c/180);

                v.z = -30 * cos(pi * b /180);
                v.x = -30 * sin(pi * b/180) ;

                v.z = v.z * cos(pi * (c+d)/180);
                v.y = 30 * sin(pi*(c+d)/180);
                double k = (-200 - cr.z)/v.z;
                point intsc = cr + v * k;
                if((abs(intsc.x) <= 80) && (abs(intsc.y) <= 80)){
                    bullets.push_back(intsc);
                    cout<<"HIT"<<endl;
                }
                //cr.print();
                //v.print();
...
point rotatePoint(point p,double angle,int axis){
    point res;
